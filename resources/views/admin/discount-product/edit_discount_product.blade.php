...
@foreach ($discount_product as $key=>$discount_product)
@foreach ($product as $key =>$product)
ID: {{$product->ProID}} 
<br>
Sản phẩm: {{$product->ProName}} 
@endforeach
<form action="{{URL::to('/update-discount-product/'.$discount_product->DisID)}}" method="post">
    {{csrf_field()}}
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="discount_value" id="discount_value" step=".1" placeholder="...%" min="0.05" max="0.7" value="{{$discount_product->Value}}" >
                    </div>
                    <div class="form-group">
                        <label class="font-weight-semibold" for="startdate">Ngày bắt đầu</label>
                        <input type="datetime-local" class="form-control" name="startdate" id="startdate" placeholder="d/m/y H:m:s" value="{{$startdate}}" required>
                    </div>
                    <div class="form-group">
                        <label class="font-weight-semibold" for="enddate">Ngày kết thúc</label>
                        <input type="datetime-local" class="form-control" name="enddate" id="startdate" placeholder="d/m/y H:m:s" value="{{$discount_product->EndDate}}" required>
                    </div>
                </div>
                <div class="m-b-15">
